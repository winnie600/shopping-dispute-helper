...
讓 C2C 平台能以更快速、透明且一致的方式處理買賣爭議，降低人工仲裁負擔，並提升整體使用者體驗。
...
Red Flags 會直接加入 LLM Prompt。

---

### 🔹 `policy.py` — 政策 Anchor 載入

將 Carousell 爭議政策以代碼表示：
...
* Fit/snugness 一律 Neutral（若尺寸標籤無誤）
* Neutral / IE 都仍需 reason（v3.2 新規定）
* 嚴格限制 JSON 格式
