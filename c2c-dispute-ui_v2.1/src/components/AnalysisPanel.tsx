    ...
    return (
      <div className="card p-6 h-full flex flex-col items-center justify-center text-center">
        ...
        <button
          onClick={onRun}
          disabled={loading}
          className="mt-3 px-4 py-2 rounded-xl bg-black text-white disabled:opacity-40"
        >
      ...
      <div className="p-4 space-y-4 text-sm">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Badge ok={result.eligibility.r1} label="R1" /> <span>Protected Channel</span>
          </div>
          <div className="flex items-center gap-2">
            <Badge ok={result.eligibility.r2} label="R2" /> <span>Within Dispute Window</span>
          </div>
          <div className="flex items-center gap-2">
            <Badge ok={result.eligibility.r3} label="R3" /> <span>Order Not Complete</span>
          </div>
        ...
        <div className="pt-2 border-t">
          <div className="flex items-center gap-2">
            <SnadLabel label={result.snadResult.label} />
            <span className="font-medium">— {result.snadResult.reason}</span>
          </div>

          {!!result.snadResult.policyAnchors?.length && (
            <div className="text-xs text-gray-500 mt-1">
              Policy anchors: {result.snadResult.policyAnchors.join(', ')}
            </div>
          )}
        </div>
        ...
        {/* Case Summary */}
        <div className="pt-2 border-t">
          <pre className="prose-block whitespace-pre-wrap">{result.caseSummary}</pre>
        </div>
      </div>
    </div>
  );
